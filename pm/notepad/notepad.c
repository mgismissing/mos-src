...
uint8_t notepad_main(coords_index start_pos) {
    char buffer[512] = {0};
    uint16_t curr_offset = 0;
    uint16_t curr_pos = 0;
    uint8_t scancode;
    m13_draw_window(start_pos, 320, 200, "NOTEPAD", notepad_img_icon, 0x0F, 0x12, 0x11, 0x13, 0);
    m13_printf(start_pos + m13_coords_to_index(2, 13), "Start typing...", 0x0E, 0xFF, 1, 0);
    m13_printf(start_pos + m13_coords_to_index(2, 190), "PRESS ESC TO EXIT. THE FILE IS TEMPORARY AND WILL NOT SAVE.", 0x0C, 0xFF, 1, 0);
    ...
    while (1) {
        ...
        if (scancode == 0x0E) {
            ...
            if (scancode == 0x1C) {
                ...
                buffer[curr_offset] = kb_scancode_to_char(scancode, 0);
                curr_offset++;
            }
        }
        curr_pos = 0;
